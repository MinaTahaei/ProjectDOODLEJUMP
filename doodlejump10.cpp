...
int showmenu_help(SDL_Surface* screen, TTF_Font* font, TTF_Font* titlefont)	
{
	int x, y;
	
	const int menunumber = 1;
	const char* menuitem[menunumber] = { "back"};//0,
	SDL_Surface* menu[menunumber] = { NULL };
	bool selected_item[menunumber] = { 0 };
	SDL_Color color[3] = { { 255,255,255 },{ 255,0,0 },{ 255,160,0 } };

	stringstream helpString[3];
	SDL_Surface * shown_help[3];
	SDL_Color helpcolor = { 200,0,70 };
	TTF_Font * helpfont = TTF_OpenFont("zachary.ttf", 30);

	SDL_Surface* title = TTF_RenderText_Solid(titlefont, "help", color[2]);
	...
	helpString[0] << "use boards to jump as high as you can!!" ;
	//	movable and stable board
	helpString[1] << "becarful!! there are broken boards too!";
	//	broken board
	helpString[2] << "and also avoid monsters and holes!"
	//	monster 1,2,3 and mirror
	
	shown_help[0] = TTF_RenderText_Solid(helpfont, helpString[0].str().c_str(), helpcolor);
	shown_help[1] = TTF_RenderText_Solid(helpfont, helpString[1].str().c_str(), helpcolor);
	shown_help[2] = TTF_RenderText_Solid(helpfont, helpString[2].str().c_str(), helpcolor);

	SDL_Rect positionhelp[3];
	
	positionhelp[0].x = 50;
	positionhelp[0].y = 200;

	positionhelp[1].x = 50;
	positionhelp[1].y = 300;

	positionhelp[2].x = 400;
	positionhelp[2].y = 50;
	...
	while (true)
	{
		...
		for(int i =0;i<3;i++)
			SDL_BlitSurface(shown_help[i], NULL, screen, &positionhelp[i]);
		...
		while (SDL_PollEvent(&event))
		{
			...
			switch (event.type)
			{
				...
				case(SDL_QUIT):
				{
					...
					for(int i =0;i<3;i++)
						SDL_FreeSurface(shown_help[i], NULL, screen, &positionhelp[i]);
					return 1;
					break;
				}
...
int showmenu_start(SDL_Surface* screen, TTF_Font* font, TTF_Font* titlefont)	//{ "play","exit"};0,1
{
	...
	const char* menuitem[menunumber] = { "play","exit","help" };//0,1,2
	SDL_Surface* menu[menunumber] = { NULL };
					...
					if (event.button.button = SDL_BUTTON_LEFT)
					{
						...
						for (int i = 0; i<menunumber; i++)
						{
							...
							if (x >= position[i].x && x <= position[i].x + position[i].w && y >= position[i].y && y <= position[i].y + position[i].h)
							{
								...
								if ( i==2 )
								{
									int j=showmenu_help(screen, font, titlefont);
									if(j==1)
										return 1;
								}
